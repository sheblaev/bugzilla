...
sub parse_mail {
    ...
    my $auto_outlook_submitted = $input_email->header('X-Auto-Response-Suppress') || '';
    if ($summary =~ /out of( the)? office/i || $auto_submitted eq 'auto-replied' || $auto_outlook_submitted eq 'All') {
